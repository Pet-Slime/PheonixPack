...
namespace PheonixPack
{
	public static class SigilUtils
	{
		...
		{
			byte[] imgBytes = File.ReadAllBytes(Path.Combine(PheonixPack.Plugin.Directory, path));
		...
		{
			return AbilityIdentifier.GetAbilityIdentifier(PheonixPack.Plugin.PluginGuid, rulebookName);
		}
